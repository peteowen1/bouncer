...
# 5. 2-way partitioning: match_type x gender (12 partitions max)
#
...
# Valid values for partition dimensions (2-way: match_type x gender)
...
#' Format: {match_type}_{gender}
#' Example: T20_male, ODI_female
...
#' Partitions by match_type x gender (up to 12 partitions)
parse_and_write_incrementally <- function(json_dir, existing_match_ids) {
  ...
  temp_innings <- file.path(TEMP_DIR, "new_innings.parquet")
  temp_powerplays <- file.path(TEMP_DIR, "new_powerplays.parquet")
  ...
  for (i in seq_along(json_files)) {
    tryCatch({
      ...
      if (!is.null(parsed$innings) && nrow(parsed$innings) > 0) {
        batch_innings[[length(batch_innings) + 1]] <- parsed$innings
      }

      if (!is.null(parsed$powerplays) && nrow(parsed$powerplays) > 0) {
        batch_powerplays[[length(batch_powerplays) + 1]] <- parsed$powerplays
      }
      ...
      if (length(batch_matches) >= BATCH_SIZE) {
        flush_batches_dynamic(
          batch_matches, batch_players, batch_innings, batch_powerplays, batch_deliveries,
          temp_matches, temp_players, temp_innings, temp_powerplays, temp_deliveries
        )
        batch_matches <<- list()
        batch_players <<- list()
        batch_innings <<- list()
  ...
  return(list(
    ...
    temp_innings = temp_innings,
    temp_powerplays = temp_powerplays,
    temp_deliveries = temp_deliveries
  ))
}

flush_batches_dynamic <- function(batch_matches, batch_players, batch_innings, batch_powerplays, batch_deliveries,
                                   temp_matches, temp_players, temp_innings, temp_powerplays, temp_deliveries) {
  ...
  if (length(batch_innings) > 0) {
    df <- do.call(rbind, batch_innings)
    if (file.exists(temp_innings)) {
      existing <- read_parquet(temp_innings)
      df <- rbind(existing, df)
      rm(existing)
    }
    write_parquet(df, temp_innings)
    rm(df)
  }

  if (length(batch_powerplays) > 0) {
    df <- do.call(rbind, batch_powerplays)
    if (file.exists(temp_powerplays)) {
      existing <- read_parquet(temp_powerplays)
      df <- rbind(existing, df)
      rm(existing)
    }
    write_parquet(df, temp_powerplays)
    rm(df)
  }
...
merge_parquets_lowmem <- function(temp_info) {
  ...
  if (file.exists(temp_info$temp_innings)) {
    new_innings <- read_parquet(temp_info$temp_innings)
  ...
  if (file.exists(temp_info$temp_powerplays)) {
    new_pp <- read_parquet(temp_info$temp_powerplays)
    if (file.exists(powerplays_path)) {
      existing_tbl <- read_parquet(powerplays_path, as_data_frame = FALSE)
      new_tbl <- arrow_table(new_pp)
      combined_tbl <- concat_tables(existing_tbl, new_tbl)
      ...
      write_parquet(new_pp, powerplays_path, compression = "zstd")
      cli_alert_success("  innings_powerplays.parquet: {nrow(new_pp)} rows (new file)")
    }
    rm(new_pp)
    gc()
  }
...
generate_manifest <- function() {
  cli_h2("Generating manifest.json")

  matches_path <- file.path(PARQUET_DIR, "matches.parquet")
  if (!file.exists(matches_path)) {
    cli_alert_warning("No matches.parquet found")
    return(NULL)
  }
...
upload_to_github <- function() {
  ...
  manifest_file <- file.path(PARQUET_DIR, "manifest.json")

  all_files <- c(parquet_files, manifest_file)
  all_files <- all_files[file.exists(all_files)]

  if (length(all_files) == 0) {
    cli_alert_warning("No files found to upload")
    return(invisible(FALSE))
  }

  total_size <- sum(file.size(all_files)) / 1024 / 1024
  cli_alert_info("Found {length(all_files)} files ({round(total_size, 1)} MB total)")
  ...
  # Upload individual files + ZIP
  cli_alert_info("Uploading {length(all_files) + 1} files...")

  for (f in all_files) {
    for (attempt in 1:3) {
      success <- tryCatch({
        pb_upload(file = f, repo = REPO, tag = TAG, overwrite = TRUE)
        TRUE
      }, error = function(e) {
  ...
  for (attempt in 1:3) {
    success <- tryCatch({
      pb_upload(file = zip_file, repo = REPO, tag = TAG, name = ZIP_NAME, overwrite = TRUE)
      TRUE
    }, error = function(e) {
      if (attempt < 3) Sys.sleep(3)
      FALSE
    })
    if (success) break
  }
...
cli_alert_info("Partitioning: match_type x gender (up to 12 partitions)")
cli_alert_info("Target release: {TAG}")
