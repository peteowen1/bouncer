# BAM Ordered Categorical Outcome Model - LONG-FORM CRICKET ----
#
...
cat("\n=== BAM Outcome Model: LONG-FORM CRICKET (Test) ===\n\n")
...
cli::cli_alert_success("Loaded {.val {nrow(model_data)}} deliveries (Test only)")
...
model_data <- model_data %>%
  mutate(
    ...
    # Phase (long-form specific - based on ball age)
    phase = case_when(
      over < 20 ~ "new_ball",      # New ball: overs 0-19
      over < 80 ~ "middle",         # Middle: overs 20-79
      TRUE ~ "old_ball"             # Old ball: overs 80+
    ),
...
# LONG-FORM specific: NO overs_left, different phases
formula <- outcome ~
  innings +                              # Categorical: 1/2/3/4
  phase +                                # Categorical: new_ball/middle/old_ball
  gender +                               # Categorical: male/female
  s(over, bs = "ts") +                  # Smooth: over number (can go to 150+)
  ...
  ti(runs_difference, wickets_fallen, bs = 'ts') +  # Interaction: pressure
  s(venue, bs = "re")                    # Random effect: venue

cli::cli_alert_info("Formula: outcome ~ innings + phase + gender + smooth terms + interactions + random(venue)")
...
cat("\n")
print(summary(bam_model_lf))
cat("\n")

# Check convergence
cli::cli_alert_info("Convergence: {.val {bam_model_lf$converged}}")

# Effective degrees of freedom
edf <- sum(bam_model_lf$edf)
cli::cli_alert_info("Total effective degrees of freedom: {.val {round(edf, 1)}}")

# Deviance explained
dev_explained <- (1 - bam_model_lf$deviance / bam_model_lf$null.deviance) * 100
...
test_probs <- predict(bam_model_lf, newdata = test_data, type = "response")
...
# Save fitted model
model_path <- file.path(models_dir, "model_bam_outcome_longform.rds")
saveRDS(bam_model_lf, model_path)
