...
team_batting_stats_remote <- function(team = NULL, match_type = NULL,
  ...
  where_clauses <- character()
  if (!is.null(team)) {
    where_clauses <- c(where_clauses, sprintf("batting_team = '%s'", team))
  }
  if (!is.null(season)) {
...
team_bowling_stats_remote <- function(team = NULL, match_type = NULL,
  ...
  where_clauses <- character()
  if (!is.null(team)) {
    where_clauses <- c(where_clauses, sprintf("bowling_team = '%s'", team))
  }
  if (!is.null(season)) {
    where_clauses <- c(where_clauses, sprintf("season = '%s'", season))
  }
...
head_to_head_remote <- function(team1, team2, match_type = NULL) {
  ...
  if (!is.null(match_type)) {
    where_clauses <- paste(where_clauses, "AND", sprintf("UPPER(match_type) = '%s'", toupper(match_type)))
  }
