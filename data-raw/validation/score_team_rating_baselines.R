...
for (fmt in fmts) {
  ...
  for (wt in c("runs", "wickets", "both")) {
    ...
    # NULL BASELINES FIRST. Comparing rating against ELO says nothing about
    # whether EITHER carries information. These are what "no knowledge of the
    # teams" scores on the same held-out matches:
    #
    #   constant  -- predict the training mean margin for every match. This is
    #                the floor any feature must clear to have earned its place.
    #   sd        -- the test set's own spread, for scale.
    #
    # If ELO barely beats the constant, then the whole rating-vs-ELO
    # comparison has been conducted inside the noise.
    r_null <- rmse(rep(mean(tr$unified_margin), nrow(te_set)), te_set$unified_margin)
    sd_te  <- stats::sd(te_set$unified_margin)
    ...
    # How much of the constant-baseline error does each feature actually remove?
    pct <- function(r) sprintf("%+.1f%%", 100 * (r_null - r) / r_null)
    cli::cli_alert_info("{toupper(fmt)}/{wt}: n={nrow(dd)} test={nrow(te_set)} | sd {round(sd_te,1)} | CONSTANT {round(r_null,2)} | ELO {round(r_e,2)} ({pct(r_e)}) | rating {round(r_r,2)} ({pct(r_r)}) | both {round(r_b,2)} ({pct(r_b)})")
    cli::cli_alert_info("   ELO-both {round(mean(bb),3)} CI [{round(ci[1],3)}, {round(ci[2],3)}] {sum(bb>0)}/2000")
  }
}
