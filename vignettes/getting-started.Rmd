...
### Option 1: Query Remote Data (No Download)

The simplest option - query parquet files directly from GitHub Releases:

```{r remote-query}
library(bouncer)

# One-liner queries (creates connection, runs query, closes)
recent_matches <- query_bouncerdata(
...
```{r remote-connection}
# Connect once, query many times
con <- connect_bouncerdata()

matches <- DBI::dbGetQuery(con, "SELECT * FROM matches LIMIT 100")
players <- DBI::dbGetQuery(con, "SELECT * FROM players LIMIT 100")
...
| Table | Description |
|-------|-------------|
| `matches` | Match metadata (teams, venue, date, result) |
| `players` | Player registry |
| `deliveries_long_form` | Ball-by-ball data for Tests, First-class |
| `deliveries_short_form` | Ball-by-ball data for T20s, ODIs |
...
```{r player-stats}
# Batting stats with derived metrics
kohli_batting <- calculate_player_batting_stats("V Kohli")
# Returns: balls_faced, runs, average, strike_rate, boundary%, elo_batting

# T20-specific batting stats
kohli_t20 <- calculate_player_batting_stats("V Kohli", match_type = "T20")

# Stats for a specific season
kohli_2023 <- calculate_player_batting_stats("V Kohli", season = "2023")

# Bowling stats
bumrah_bowling <- calculate_player_bowling_stats("JJ Bumrah")
# Returns: balls_bowled, runs, wickets, average, economy, strike_rate, elo_bowling
